...
class PowerUps {
    ...
    smallBatPower(player1: Bat, player2: Bat) {
        if (this.turn === 0) player2.height = this.canvas.width / 10;
        if (this.turn === 1) {
    ...
    getRandomPower(
        ...
        if (this.power === true && !this.powerTaken) {
            ...
            else if (randomNum === 2) this.tinyBallPower(pongball);
            else if (randomNum === 3) this.fastBallPower(pongball);
            else this.duplicateBallPower(pongballpower);
        }
        if (!this.powerTaken) {
